...
static void handle_incoming_button_press(const esp_now_recv_info_t *recv_info, const message_t *msg) {
    ...
    led_control_queue_command(1, 100, true); // High priority for button press indication
}
...
static void complete_pairing(const uint8_t *mac_addr) {
    ...
    led_control_queue_command(2, 100, true); // High priority for pairing success
...
static void unpair_device(void) {
    ...
    led_control_queue_command(5, 150, true); // High priority for unpair indication
...
static void pairing_broadcast_timer_cb(TimerHandle_t xTimer) {
    if (s_is_pairing) {
        ...
        led_control_queue_command(1, 1000, false); // Low priority for pairing broadcast indication
    }
}

// Button press callbacks
static void button_single_click_cb(void *arg, void *usr_data) {
    led_control_queue_command(1, 50, true); // High priority for button press indication
    if (s_is_paired) {
...
static void gpio_init(void) {
    ...
    led_config_t led_config = LED_CONFIG_DEFAULT();
    led_config.led_gpio = OUTPUT_LED_GPIO;
    
    esp_err_t ret = led_control_init(&led_config);
    if (ret != ESP_OK) {
